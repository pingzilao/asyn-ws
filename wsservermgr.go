...
func (this *WsServerMgr) HandleWsConnection(con sws.Connection) {
	...
	con.OnMessage(func(data []byte) {
		...
		if err != nil {
			...
			this.Json(con, this.mNewRequesterHandler(ws, nil, "", this.RemoveReq, this.mPackPushHandler, this.mPackResHandler), ErrCode_UrlPath, nil)
			return
		}
		...
		if requester.IsSub() { //先设置，万一失败了再删除，这样做可以让防止推送数据丢失
			if len(requester.GetQid()) == 0 {
				...
				this.Json(con, requester, ErrCode_UrlPath, nil)
				return
			}
			if err = this.AddReq(requester); err != nil {
				...
				this.Json(con, requester, ErrCode_InerServer, nil)
				return
			}
		...
		ZapLog().Info("find path " + requester.GetPath())
